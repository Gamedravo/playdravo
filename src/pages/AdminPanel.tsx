...
export const AdminPanel: React.FC<AdminPanelProps> = ({ isDarkMode, t, type }) => {
  ...
  useEffect(() => {
    const collectionName = type === 'bug-reports' ? 'bugReports' : type === 'game-requests' ? 'gameRequests' : 'contactMessages';
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
          ...
          <div className="flex items-center gap-4 flex-wrap">
            <Link 
              ...
              className={`px-6 py-3 rounded-xl text-[10px] font-semibold tracking-wide transition-all ${type === 'bug-reports' ? 'bg-accent text-bg-dark' : (isDarkMode ? 'bg-white/5 text-white/60 hover:bg-white/10' : 'bg-black/5 text-black/60 hover:bg-black/10')}`}
            >
              Bug Reports
            </Link>
            <Link 
              to="/admin/game-requests"
              className={`px-6 py-3 rounded-xl text-[10px] font-semibold tracking-wide transition-all ${type === 'game-requests' ? 'bg-accent text-bg-dark' : (isDarkMode ? 'bg-white/5 text-white/60 hover:bg-white/10' : 'bg-black/5 text-black/60 hover:bg-black/10')}`}
            >
              Game Requests
            </Link>
            <Link 
              to="/admin/support-tickets"
              className={`px-6 py-3 rounded-xl text-[10px] font-semibold tracking-wide transition-all ${type === 'support-tickets' ? 'bg-accent text-bg-dark' : (isDarkMode ? 'bg-white/5 text-white/60 hover:bg-white/10' : 'bg-black/5 text-black/60 hover:bg-black/10')}`}
            >
              Support Tickets
            </Link>
          </div>
        ...
        <div className={`mb-8 p-4 rounded-[2rem] border flex flex-col md:flex-row items-center gap-4 ${isDarkMode ? 'bg-white/[0.02] border-white/5' : 'bg-black/[0.02] border-black/5'}`}>
          <div className="flex-1 relative w-full">
            ...
            <input 
              ...
              className={`w-full bg-transparent border-none outline-none pl-12 pr-4 py-2 text-sm font-medium ${isDarkMode ? 'text-white placeholder:text-white/20' : 'text-black placeholder:text-black/20'}`}
            />
          </div>
          ...
          <button 
            onClick={() => {
              ...
              else if (sortBy === 'oldest') setSortBy('unread');
              else setSortBy('newest');
            }}
            className={`flex items-center gap-2 px-4 py-2 rounded-xl transition-all ${isDarkMode ? 'hover:bg-white/5' : 'hover:bg-black/5'}`}
          >
            <Filter className={`w-4 h-4 ${isDarkMode ? 'text-white/20' : 'text-black/20'}`} />
            <span className={`text-[10px] font-semibold tracking-wide ${isDarkMode ? 'text-white/60' : 'text-black/60'}`}>
              {sortBy === 'newest' ? 'Newest First' : sortBy === 'oldest' ? 'Oldest First' : 'Unread First'}
            </span>
          </button>
        </div>
        ...
        <div className="space-y-4">
          {loading ? (
            <div className="py-24 text-center">
              ...
              <p className={`text-[10px] font-semibold tracking-wide ${isDarkMode ? 'text-white/40' : 'text-black/40'}`}>Synchronizing Data...</p>
            </div>
            ...
            <AnimatePresence mode="popLayout">
              {filteredSubmissions.map((sub, idx) => (
                <motion.div
                  ...
                  className={`p-6 md:p-8 rounded-[2.5rem] border group transition-all duration-300 hover:border-accent/30 ${isDarkMode ? 'bg-white/[0.02] border-white/5 hover:bg-white/[0.04]' : 'bg-black/[0.02] border-black/5 hover:bg-black/[0.04]'}`}
                >
                  <div className={`flex flex-col md:flex-row md:items-start justify-between gap-6 ${sub.read ? 'opacity-60' : 'opacity-100'}`}>
                    <div className="flex-1 space-y-4">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-4 mt-1">
                            <div className="flex items-center gap-2">
                              ...
                              <span className={`text-[10px] font-semibold tracking-wide ${isDarkMode ? 'text-white/40' : 'text-black/40'}`}>
                      ...
                      {sub.link && (
                        <a 
                          ...
                          className="inline-flex items-center gap-2 text-accent hover:underline text-xs font-bold"
                        >
                    ...
                    <div className="flex md:flex-col items-center gap-2">
                      <button 
                        onClick={() => toggleReadStatus(sub.id, !!sub.read)}
                        title={sub.read ? "Mark as unread" : "Mark as read"}
                        className={`p-3 rounded-xl transition-all ${sub.read ? 'bg-emerald-500/20 text-emerald-500' : (isDarkMode ? 'bg-white/5 hover:bg-emerald-500/20 text-white/40 hover:text-emerald-500' : 'bg-black/5 hover:bg-emerald-500/20 text-black/40 hover:text-emerald-500')}`}
                      >
                      ...
                      {sub.email && (
                        <a 
                          href={`mailto:${sub.email}?subject=Re: ${type === 'support-tickets' ? sub.subject : sub.gameName}`}
                      ...
                      {deletingId === sub.id ? (
                        <div className="flex flex-col gap-1">
                          <button 
                            ...
                            className="p-2 text-[10px] font-bold uppercase tracking-wider bg-red-500 text-white rounded-lg hover:bg-red-600 transition-colors"
                          >
                            Confirm
                          </button>
                          <button 
                            onClick={() => setDeletingId(null)} 
                            className={`p-2 text-[10px] font-bold uppercase tracking-wider rounded-lg transition-colors ${isDarkMode ? 'bg-white/10 text-white hover:bg-white/20' : 'bg-black/10 text-black hover:bg-black/20'}`}
                          >
                            Cancel
                          </button>
                        </div>
                        ...
                        <button 
                          ...
                          className={`p-3 rounded-xl transition-all ${isDarkMode ? 'bg-white/5 hover:bg-red-500/20 text-white/40 hover:text-red-500' : 'bg-black/5 hover:bg-red-500/20 text-black/40 hover:text-red-500'}`}
                        >
